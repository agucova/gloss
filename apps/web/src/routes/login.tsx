...
// Dev impersonation panel — two static guards, either enough on its own to
// tree-shake the panel module out of the prod bundle:
//   1. VITE_DEV_IMPERSONATION must be "true" (set only in dev .env)
//   2. Vite must NOT be in production mode
// Both halves become compile-time constants; when either is false Rollup
// evaluates the ternary to null and drops the dynamic import, which in turn
// makes dev-impersonation-panel.tsx unreferenced across modules.
const DevImpersonationPanel =
	import.meta.env.VITE_DEV_IMPERSONATION === "true" && !import.meta.env.PROD
		? lazy(() =>
				import("@/components/dev-impersonation-panel").then((m) => ({
					default: m.DevImpersonationPanel,
				}))
			)
		: null;
	...
	return (
		<div className="flex min-h-[calc(100vh-4rem)] flex-col items-center justify-center px-6 py-16">
			...
			<div className="relative w-full max-w-sm">
				...
				{/* Dev impersonation panel — compile-time stripped in prod builds */}
