...
async function searchImpl(
	...
	if (types.includes("highlight")) {
		...
		for (const h of hits) {
			if (!isVisible(h.userId, h.visibility, userId, friendIdSet)) continue;
	...
	if (types.includes("bookmark")) {
		const hits = await ctx.db
			.query("bookmarks")
			.withSearchIndex("search_content", (q) =>
				q.search("searchContent", args.q).eq("userId", userId)
			)
			.take(limit * 2);
	...
	if (types.includes("comment")) {
		...
		for (const c of hits) {
			...
			if (
				!highlight ||
				!isVisible(highlight.userId, highlight.visibility, userId, friendIdSet)
			)
				continue;
...
export const search = query({
	...
	handler: async (ctx, args) => {
		...
		return searchImpl(ctx, args, auth.userId);
	},
});

// Internal variant for API-key-authenticated HTTP actions.
export const searchByUserInternal = internalQuery({
	args: {
		userId: v.id("users"),
		q: v.string(),
		limit: v.optional(v.number()),
	...
	handler: async (ctx, args) => {
		...
		return searchImpl(ctx, rest, userId);
	},
});
